...
try:
    ...
    conflict_clause = "ON CONFLICT (label) DO NOTHING" if Config.DB_TYPE == 'postgres' else ""
    insert_offers_sql = f"""
    ...
    conn.commit()
    print("Default offers inserted/verified successfully.")
    ...
    for offer in offers:
        # Note: offer[2] corresponds to description in both structures
        description = offer[2]
        if isinstance(description, str):
            description = description.encode('ascii', 'replace').decode('ascii')
        print(f"  - {offer[1]}: {description[:50]}...")
